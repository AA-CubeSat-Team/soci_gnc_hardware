...
{
  standby();
  writeField(FXAS21002C_H_F_SETUP, 6, 2, 0b01);
  ready();
...
void readGyroData(float gyroX[], float gyroY[], float gyroZ[], int8_t *tempData)
{
  ...
  uint8_t rawData[30];  // x/y/z gyro register data stored here
  for (int i = 0; i<5; i++){
  readRegs(FXAS21002C_H_OUT_X_MSB, 6, &rawData[6*i]);  // Read the six raw data registers into data array
  }
  for (int i = 0; i<5; i++){
    gyroX[i] = ((int16_t)(((int16_t)rawData[6*i]) << 8 | ((int16_t) rawData[6*i + 1])));
    gyroY[i] = ((int16_t)(((int16_t)rawData[6*i + 2]) << 8 | ((int16_t) rawData[6*i + 3])));
    gyroZ[i] = ((int16_t)(((int16_t)rawData[6*i + 4]) << 8 | ((int16_t) rawData[6*i + 5])));

    #if COUNT_TEMP_BIAS
    gyroX[i] = (gyroX[i])*SENSITIVITY*(1 + TEMP_SENS_COE_X*(int16_t) tempDelta) - gBiasX - TEMP_BIAS_COE_X*(int16_t) tempDelta;
    gyroY[i] = (gyroY[i])*SENSITIVITY*(1 + TEMP_SENS_COE_Y*(int16_t) tempDelta) - gBiasY - TEMP_BIAS_COE_Y*(int16_t) tempDelta;
    gyroZ[i] = (gyroZ[i])*SENSITIVITY*(1 + TEMP_SENS_COE_X*(int16_t) tempDelta) - gBiasZ - TEMP_BIAS_COE_Z*(int16_t) tempDelta;
    #else
    gyroX[i] = (gyroX[i])*SENSITIVITY - gBiasX;
    gyroY[i] = (gyroY[i])*SENSITIVITY - gBiasY;
    gyroZ[i] = (gyroZ[i])*SENSITIVITY - gBiasZ;  
    #endif
  }
