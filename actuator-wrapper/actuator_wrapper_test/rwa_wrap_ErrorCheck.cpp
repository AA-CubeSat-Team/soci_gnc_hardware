...
// Error checks
void ErrorCheck ( struct rw_data *rwX_pt, uint8_t SS){
  if rwX_pt->state == 0{
    // error present
    // Initialize RWA, return to nominal state
    commandIndvidual(5, wheelNum, rwX_pt, SS) // replacment for commandAll, for a reset to act on an individual wheel
  }
  if rwX_pt->lastResetStatus != 6 && rwX_pt->lastResetStatus != 7{
    commandIndvidual(3, wheelNum, rwX_pt, SS)
    commandIndvidual(7, wheelNum, rwX_pt, SS)
  }
}



// arguments need to be assigned for commandIndividual function

void commandIndividual(uint8_t com_id, uint8_t wheelNum, struct rw_data *rwX_pt, uint8_t SS)
  ...
  reqPayloadWriteSwitcher(com_id, &req_payload_rwX[0], &req_payload_len_rwX, &rwX);
  reqPacketProcess(&req_payload_rwX[0], &req_packet_rwX[0], &req_payload_len_rwX, &req_packet_len_rwX);
  reqSpiTransfer(&req_packet_rwX[0], &req_packet_len_rwX, SS);

  delay(SPI_TIMEOUT);
  
  rplSpiTransfer(&rpl_packet_rwX[0], &rpl_packet_len_rwX, SS);
  rplPacketProcess(&rpl_payload_rwX[0], &rpl_packet_rwX[0], &rpl_payload_len_rwX, &rpl_packet_len_rwX);
  rplPayloadReadSwitcher(com_id, &rpl_payload_rwX[0], &rpl_payload_len_rwX, &rwX);
...
// command 1 - reset MCU  --- --- --- --- ---
void reqPayloadWrite_cmd1(uint8_t *req_payload_pt, uint8_t *req_payload_len_pt, struct rw_data *rwX_pt) {
  uint8_t com_id = 1;
...
void rplPayloadRead_cmd1(uint8_t *rpl_payload_pt, uint8_t *rpl_payload_len_pt, struct rw_data *rwX_pt) { 
  // com_id = 1
  // length = 2
...
// command 5 - initialize reaction wheel controller --- --- --- --- ---
